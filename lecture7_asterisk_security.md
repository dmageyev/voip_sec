...
| 2 | Принцип мінімізації привілеїв (PoLP): ОС, ФС, мережа |
| 3 | ACL: контроль доступу на рівні IP |
| 4 | Контексти SIP-драйверів: ізоляція груп |
| 5 | Безпека диалплану: Toll Fraud, DISA, валідація |
| 6 | Автентифікація, TLS/SRTP, AMI, моніторинг |
| 7 | Типові помилки, чек-ліст, Q&A |
...
## 🔐 Принцип мінімізації привілеїв (PoLP)

> **Principle of Least Privilege:** кожен компонент системи отримує лише ті права, які мінімально необхідні для виконання своєї функції.

**Рівні застосування в IP-АТС:**

```
┌─────────────────────────────────────────┐
│  Диалплан: тільки дозволені напрямки    │  ← Рівень 4
├─────────────────────────────────────────┤
│  Asterisk ACL: permit/deny по IP        │  ← Рівень 3
├─────────────────────────────────────────┤
│  Мережа: VLAN, iptables, fail2ban       │  ← Рівень 2
├─────────────────────────────────────────┤
│  ОС: непривілейований користувач       │  ← Рівень 1
└─────────────────────────────────────────┘
```

**Принцип Defense-in-Depth:** порушення одного рівня не компрометує всю систему.
...
## 📌 Підсумок: ключові принципи

**1. Defense-in-Depth**
Кожен рівень (ОС → мережа → Asterisk ACL → контекст → диалплан) є окремим рубежем захисту.

**2. Мінімізація привілеїв**
Кожен акаунт, контекст, AMI-користувач отримує мінімум прав, необхідних для роботи.

**3. Whitelist замість Blacklist**
Заборонити все, дозволити явно — надійніше, ніж намагатись заблокувати всі загрози.

**4. Моніторинг і реакція**
Безпека без моніторингу неповна. SECURITY-логи + SIEM + алерти.

**5. Регулярний аудит**
Конфігурація Asterisk змінюється — щомісячний перегляд чек-листа обов'язковий.
